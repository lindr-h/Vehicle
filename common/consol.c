...
void CONSOL_Init(int wBaud)
{
        ...
        {
        case 0:
                rUFCON0 = 0x0;   //UART channel 0 FIFO control register
                rUMCON0 = 0x0;
                rULCON0 = 0x3;   //Line control register : Normal,No parity,1 stop,8 bits
                rUCON0  = 0x245; // Control register
                rUBRDIV0=( (int)(PCLK/16./wBaud+0.5) -1 );   //Baud rate divisior register 0
                break;

        case 1:
                rUFCON1 = 0;   //UART channel 0 FIFO control register
                rUMCON1 = 0;
                rULCON1 = 0x3;   //Line control register : Normal,No parity,1 stop,8 bits
                rUCON1  = 0x245; // Control register
                rUBRDIV1=( (int)(PCLK/16./wBaud+0.5) -1 );   //Baud rate divisior register 0
                break;

        case 2:
                rUFCON2 = 0;   //UART channel 0 FIFO control register
                rULCON2 = 0x3;   //Line control register : Normal,No parity,1 stop,8 bits
                rUCON2  = 0x245; // Control register
                rUBRDIV2=( (int)(PCLK/16./wBaud+0.5) -1 );   //Baud rate divisior register 0
                break;
        }
}

//    port1.com_port   =  SD_UART1; 
//    port1.baud_rate  = 9600; 
//    port1.data_bits  = DATA_BITS_8; 
//    port1.stop_bits  = STOP_BITS_1; 
//    port1.parity     = SD_PARITY_ODD; 
//    port1.data_mode  = DEFAULT_UART_MODE;
//    port1.communication_mode = SERIAL_MODE;
//    port1.sd_buffer_size   =   DEFAULT_UART_BUFFER;
//    status = SDC_Init_Port (&port1);

void CONSOL_InitUart1(int wBaud)
{

                rUFCON1 = 0;   //UART channel 0 FIFO control register
                rUMCON1 = 0;
                rULCON1 = 0x23;   //Line control register : Normal,No parity,1 stop,8 bits
                rUCON1  = 0x245; // Control register
                rUBRDIV1=( (int)(PCLK/16./wBaud+0.5) -1 );   //Baud rate divisior register 0
}
...
char CONSOL_GetCh(void)
{
        ...
        {
        case 0:
                while(!(rUFSTAT0 & 0x1)); //Receive data ready
                return RdURXH0();

        case 1:
                while(!(rUFSTAT1 & 0x1)); //Receive data ready
                return RdURXH1();

        case 2:
                while(!(rUFSTAT2 & 0x1)); //Receive data ready
                return RdURXH2();
        }
        return RdURXH0();
}
